  ...
  return (
    <div className="relative w-full overflow-hidden py-4">
      ...
      <div className="flex flex-col gap-6">
        <div className="flex w-max animate-marquee-right gap-6" style={{ animationDuration: "20s" }}>
          {repeatedRow1.map((t, i) => (
            <TestimonialCard key={`r1-${t.id_testimoni}-${i}`} t={t} />
          ))}
        </div>

        <div className="flex w-max animate-marquee-left gap-6" style={{ animationDuration: "20s" }}>
...
export default function Home() {
  ...
  return (
    <div className="space-y-12 pb-20">
      {/* ══════════════════════════════ HERO ══════════════════════════════ */}
      <section className="relative overflow-hidden bg-[#0A2E26] flex items-center min-h-[460px] sm:min-h-[500px] lg:min-h-[560px] py-12 sm:py-0">
        {settings.banner_hero && (
          <img src={`http://localhost:8000/storage/${settings.banner_hero}`} alt="Hero Banner" className="absolute inset-0 w-full h-full object-cover" width="1280" height="480" fetchPriority="high" loading="eager" decoding="async" />
        )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
          <div className="text-left md:max-w-2xl space-y-5 sm:space-y-6" style={{ animation: 'heroFadeUp 0.8s ease-out both' }}>
